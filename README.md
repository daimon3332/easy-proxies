...
- 订阅刷新支持逐 URL 进度追踪；某个订阅 URL 刷新失败时，该 URL 对应旧节点会整体转入失败节点池。
...
11. 如果某个订阅 URL 在 3 分钟内始终没有拉取到任何节点，或者后续 commit / 测速流程失败，则该 URL 旧节点会整体转入失败节点池，不会回退到节点池。

手动刷新不会把新旧订阅结果做并集，而是以本次成功解析和测速后的最新结果为准。

### Windows 联动启动脚本

仓库内提供 `start_easy_outlook.ps1`，用于串联 Easy Proxies 与 Outlook 注册脚本：

1. 启动 `easy_proxies`
2. 等待 WebUI / 管理 API 就绪
3. 调用 `/api/import/refresh` 刷新全部订阅来源
4. 持续轮询刷新任务，直到所有订阅 URL 进入终态
5. 读取最终真实节点池数量
6. 按 `OutlookRegister/config.json` 中的 `proxy.port_start + 节点池数量` 更新 `proxy.port_end`
7. 如果节点池为空则直接退出，不启动 Outlook 脚本
8. 如果节点池非空，则继续启动 `python D:\Outlook\OutlookRegister\main.py`

注意：刷新任务整体状态即使是 `failed`，脚本仍会继续检查最终真实节点池；只有节点池为空时才会终止后续启动。
...
| `POST` | `/api/import/refresh` | 启动全部或单个导入来源的订阅刷新任务 |
| `GET` | `/api/import/refresh/jobs/{job_id}` | 查询订阅刷新任务进度 |
...
如果某个订阅 URL 刷新失败，该 URL 对应的旧节点不会留在节点池，也不会回退入池，而是会整体转入失败节点池，等待后续手动或自动重测。
