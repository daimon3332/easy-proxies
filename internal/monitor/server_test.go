...
func TestHandleIndexIncludesRefreshProgressUI(t *testing.T) {
	...
	for _, marker := range []string{"easy_proxies_active_refresh_job", "failed_nodes", "refresh-modal", "probe_round_done", "重新检测部分完成", "恢复检测前节点池", "测速已终止，检测前节点池已恢复", "return await retestNodes([id])"} {
